...
def system_cmd(echo, cmd, file_name) :
	...
	if flag != 0 :
		if not echo :
			print(command)
		sys.exit('speed_old.py: command failed' )
	if file_out != None :
		file_out.close()
	#
	return
...
# ---------------------------------------------------------------------------
# main program
usage  = '''usage: bin/speed_old.py option
option: is either 'build', 'run', 'summary'
'''
if len(sys.argv) != 2 :
	sys.exit(usage)
if sys.argv[0] != 'bin/speed_old.py' :
	sys.exit(usage)
#
...
if option == 'build' :
	...
	for program in [ 'ar1_xam', 'capture_xam' ] :
		...
		print('')
# ---------------------------------------------------------------------------
print('bin/speed_old.py ' + sys.argv[1] + ': OK')
