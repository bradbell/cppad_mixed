...
// ----------------------------------------------------------------------------
// Test case that was failing during finalize solution
// BEGIN C++
...
namespace {
   ...
   public:
      ...
      {  typedef typename Vector::value_type scalar;
         ...
         {
            // Data term
            scalar res  = (z_[j] - fixed_vec[j]) / std_[j];
...
bool finalize(void)
{
   ...
   mixed_derived mixed_object(
      n_fixed, n_random, quasi_fixed, bool_sparsity, A_rcv, z, std
   );
   ...
   {
      ok &= fabs( fixed_out[j] / z[j] - 1.0 ) < 1e-2;
      ok &= fixed_lag[j] == 0.0;
   }
   return ok;
}
