...
$children%
	example/private/fix_likelihood_hes_xam.cpp
%$$
$head Example$$
The file $cref fix_likelihood_hes_xam.cpp$$ contains an example
...
{	assert( init_fix_likelihood_done_ );
	...
	if( row_out.size() == 0 )
	{	row_out = fix_likelihood_hes_.row;
		col_out = fix_likelihood_hes_.col;
		val_out.resize( row_out.size() );
	}
# ifndef NDEBUG
	else
	{	size_t n_nonzero = fix_likelihood_hes_.row.size();
		assert( row_out.size() == n_nonzero );
		for(size_t k = 0; k < n_nonzero; k++)
		{	assert( row_out[k] == fix_likelihood_hes_.row[k] );
			assert( col_out[k] == fix_likelihood_hes_.col[k] );
		}
	}
# endif
