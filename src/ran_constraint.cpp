...
$head Syntax$$
$icode%mixed_object%.init_ran_con_mat(%n_random%, %row%, %col%, %val%)
%$$
...
in $icode A$$.
Note that if $icode%K% == 0%$$, then the matrix $icode A$$ is empty.
...
in $icode A$$.
Note that for each $icode k$$, $codei%col%[%k%]% < %n_random%$$.
...
{	assert( n_random > 0 );
	...
	// put the values in A
	for(size_t k = 0; k < K; k++)
		ran_con_mat_.insert(row[k], col[k]) = val[k];
