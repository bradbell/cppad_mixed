...
(
    ...
    const Int *Lp,    /* L is sparse, lower triangular, stored by column */
    const Int *Li,    /* the row indices of L must be sorted */
    const double *Lx, /* diagonal of L, if present, is ignored */

    const double *d,  /* diagonal of D, of size n */

    const Int *Up,    /* U is sparse, upper triangular, stored by row */
    const Int *Uj,    /* the column indices of U need not be sorted */
    const double *Ux, /* diagonal of U, if present, is ignored */

    const Int *Zp,    /* Z is sparse, stored by column */
    const Int *Zi,    /* the row indices of Z must be sorted */
