...
# {xrst_comment_ch #}
#
# tools/run_cmake.sh: User Configuration Options
# ##############################################
#
# verbose_makefile
# ****************
...
# The example install script ``tools/install_eigen.sh`` uses
...
#
# | |tab| ``tools/install_cppad.sh``
# | |tab| ``tools/run_cmake.sh``
# | |tab| ``cd build`` ; ``make`` *program* ; ``cd ..``
# | |tab| ``tools/`` *program* . ``sh`` *test2run*
#
# where *program* is
...
then
    echo "tools/run_cmake.sh: build_type is not 'debug' or 'release'"
    exit 1
fi
while [ "$1" != '' ]
do
...
tools/build_type.sh run_cmake $build_type
# --------------------------------------------------------------------------
