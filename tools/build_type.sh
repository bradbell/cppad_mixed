...
set -e -u
#
if [ ! -e "tools/build_type.sh" ]
then
    echo 'tools/build_type.sh: must be executed from its parent directory.'
    exit 1
fi
if [ "$#" != 2 ]
then
    cat << EOF
tools/build_type.sh program_name build_type

program_name:         is the program name for error reporting.
build_type:           this must be either debug or release
cmake_install_prefix: is its value in tools/run_cmake.sh
...
eval $(grep '^cmake_install_prefix=' tools/run_cmake.sh)
#
# total_prefix
dollar='$'
