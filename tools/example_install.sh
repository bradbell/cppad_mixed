...
# Syntax
# ******
# ``tools/example_install.sh`` *run_test* *replace*
#
# run_test
# ********
...
#
# BEGIN BASH
if [ $0 != 'tools/example_install.sh' ]
then
    echo 'tools/example_install.sh: must be executed from its parent directory'
    exit 1
fi
# -----------------------------------------------------------------------------
...
if [ "$#" != 2 ]
then
    echo 'tools/example_install.sh: run_test replace'
...
cmd=`grep '^build_type=' tools/run_cmake.sh`
eval $cmd
...
then
    tools/build_type.sh example_install.sh $build_type
fi
# --------------------------------------------------------------------------
user=$(whoami)
if [ "$user" == 'root' ]
...
do
    ...
    case $pkg in
        ...
        *)
        echo 'tools/example_install.sh: program error'
        exit 1
        ;;
    esac
    #
    ...
    then
        if [ "$replace" == 'false' ]
        then
            echo "Skipping tools/install_$pkg.sh"
    ...
    then
        ...
        echo "tools/install_$pkg.sh $system_type 1>> $name.log 2>> $name.err"
        if ! tools/install_$pkg.sh $system_type 1>> $name.log 2>> $name.err
        then
...
if ! tools/run_cmake.sh 1>> example_install.log 2>> example_install.err
then
