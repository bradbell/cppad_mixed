...
# ----------------------------------------------------------------------------
if [ $0 != 'tools/install_ipopt.sh' ]
then
    echo 'tools/install_ipopt.sh: must be executed from its parent directory'
    exit 1
fi
#
# system_type
...
then
    echo 'tools/install_ipopt.sh: system_type'
...
#
# specific_compiler
cmd=`grep '^specific_compiler=' tools/run_cmake.sh`
eval $cmd
#
...
#
# build_type.sh
tools/build_type.sh install_ipopt $build_type
#
# external/$build_type
