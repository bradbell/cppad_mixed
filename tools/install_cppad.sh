...
# ----------------------------------------------------------------------------
if [ $0 != 'tools/install_cppad.sh' ]
then
    echo 'tools/install_cppad.sh: must be executed from its parent directory'
    exit 1
fi
# -----------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Use same version and hash as in cppad_py.git/tools/get_cppad.sh
...
#
# extra_cxx_flags
cmd=`grep '^extra_cxx_flags=' tools/run_cmake.sh`
eval $cmd
#
# cmake_libdir
...
then
    tools/build_type.sh install_cppad $build_type
fi
# --------------------------------------------------------------------------
