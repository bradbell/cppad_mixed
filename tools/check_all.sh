...
#
if ! $grep "build_type='debug'" tools/run_cmake.sh > /dev/null
then
    echo 'tools/check_all.sh: bin/run_cmake.sh build_type not debug'
    echo 'Attempted to automatically fix this.'
    restore_and_exit 1
fi
if ! $grep "optimize_cppad_function='no'" tools/run_cmake.sh > /dev/null
then
    echo 'tools/check_all.sh: bin/run_cmake.sh optimize_cppad_function not no'
    restore_and_exit 1
fi
#
if [ $# == 1 ]
...
if [ "$release" == 'yes' ]
then
    $sed -i tools/run_cmake.sh -e "s|^build_type=.*|build_type='release'|"
fi
#
# typos
...
#
# tools/check_*.sh
list=`ls tools/check_*.sh`
for script in $list
do
    if [ "$script" != 'tools/check_all.sh' ] \
    && [ "$script" != 'tools/check_copy.sh' ] \
    && [ "$script" != 'tools/check_install.sh' ]
    then
        $script
    fi
done
...
cmd=`$grep '^cmake_install_prefix=' tools/run_cmake.sh`
eval $cmd
#
# cmake_libdir
cmd=`$grep '^cmake_libdir=' tools/run_cmake.sh`
eval $cmd
...
echo "tools/run_cmake.sh $flags >& cmake.log"
tools/run_cmake.sh $flags >& cmake.log
#
# run_xrst.sh
flags=''
...
cd ..
#
if ! tools/check_install.sh
then
    echo 'tools/check_install.sh: Error'
    exit 1
fi
#
# warnings
...
do
    ...
    then
        echo "tools/run_check_all.sh: $target.log is has warnings."
        exit 1
    fi
done
# -----------------------------------------------------------------------------
