...
# ----------------------------------------------------------------------------
if [ ! -e 'tools/speed_new.sh' ]
then
    echo 'tools/speed_new.sh must be run from its parent directory'
    exit 1
fi
ok='yes'
...
if [ "$ok" == 'no' ]
then
    echo 'usage: tools/speed_new.sh test2run quasi_fixed'
...
# -----------------------------------------------------------------------------
git reset --hard
tools/run_cmake.sh --release --optimize_cppad_function
# -----------------------------------------------------------------------------
for program in ar1_xam capture_xam
do
    diff=`git diff tools/$program.sh`
    if [ "$diff" != '' ]
    then
        echo "tools/speed_new.sh: bin/$program.sh has changed"
        exit 1
    fi
    for ver in old new
    do
...
do
    ...
    do
        ...
        #
        echo "tools/$program.sh $test2run > build/$ver.$program.out"
        tools/$program.sh $test2run > build/$ver.$program.out
        if [ "$test2run" == 'massif' ]
        then
    ...
    # This may overwrite tools/$program.sh
    git_new.sh from
done
# -----------------------------------------------------------------------------
